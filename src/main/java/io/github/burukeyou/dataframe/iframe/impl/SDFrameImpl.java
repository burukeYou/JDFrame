...
public class SDFrameImpl<T>  extends AbstractDataFrameImpl<T> implements SDFrame<T> {
    ...
    public SDFrameImpl(Stream<T> data) {
        ...
        if (ListUtils.isNotEmpty(tmp)){
            ...
            first.ifPresent(t -> fieldClass = t.getClass());
        }
    ...
    public SDFrameImpl(List<T> list) {
        ...
        if (!list.isEmpty()){
            if (list.get(0) != null){
                ...
                list.stream().filter(Objects::nonNull).findFirst().ifPresent(e -> fieldClass = e.getClass());
            }
        }
    }
    ...
    public SDFrameImpl<T> subtract(IFrame<T> other, Comparator<T> comparator) {
        return returnDF(subtractList(viewList(),other.toLists(),comparator));
    }
     ...
     * But for SDFrame design is all about sharing one, Unless otherwise specified
     *
     */
